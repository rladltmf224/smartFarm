...
// import {
//   LookUpGrowthResearchData,
//   SaveGrowthResearchMemo,
//   DeleteGrowthResearchDate,
//   GetGrowthResearchGraphData,
// } from "@/api/index.js";
...
export default {
  ...
  data() {
    return {
      deletePageNum: 0, //자식에서 보낸 삭제할 페이지 고유 num
      graphData: [],
      graph_dialog: false,
      growthReportId: "",
      model: null,
      zoomItemName: "", //이미지 확대기능
      zoomItemUrl: "", //이미지 확대기능
      emptyImageText: false, //이미지 없을때 대체 텍스트 (임시)
      selectedImage: [], //이미지 개별삭제
      ...
      deleteDialog: false, //현재페이지를 삭제하시겠습니까?다이아로그
      deleteImageDialog: false, //선택된 사진을  삭제하시겠습니까?다이아로그
      nowPageData: [], //현재페이지의 데이터를 담는 변수
      ApiResult: [], //api결과데이터
      numDay: 0,
      growthReportDetailId: 0, //날짜 수정및 등록할때 필요한 날짜디테일id
      numTreatment: 1, //처리구 개수
      numSample: 1, //개체수
      number: 1, // 캐러샐 개수
      table: [], // 화면구성하는 기본데이터
      detail: [],
      ...
      text: "저장되었습니다.",
      timeout: 1000,
      snackbar_date: false, //데이터 저장 후 스낵바
      text_date: "날짜가 등록되었습니다.", //날짜 저장 후 스낵바
      delete_text: "삭제되었습니다.", //삭제시 스낵바 텍스트
      disable: false, // disable항목
      dialog: false, // 사진등록 다이아로그
      testCondition: "",
      testName: "",
    };
  },
  ...
  methods: {
    ...
    getData(callChildSave = false) {
      ...
      api.growthresearch.LookUpGrowthResearchData(id).then((res) => {
        ...
        if (this.ApiResult.details == null) {
          ...
          this.ApiResult.details = this.ApiResult.details.sort(function (a, b) {
          ...
          for (
            ...
            if (this.ApiResult.details[DetailNum].treatments.length == 0) {
              ...
              treatmentNameUniqList.forEach(function (tr) {
                let trList = _.filter(pageItem.treatments, {
                  treatmentName: tr,
                });
    ...
    getGraphData() {
      ...
      api.growthresearch.GetGrowthResearchGraphData(id).then((res) => {
        ...
        setObj_avgLeafHeight.forEach((el) => {
          //5.이중 forEach써서 name과treatmentName이같으면 avgInternodeLength값을 넣어준다.
          fullData.forEach((el2) => {
            if (el2.treatmentName == el.name) {
        ...
        setObj_avgLeafLength.forEach((el) => {
          //5.이중 forEach써서 name과treatmentName이같으면 avgInternodeLength값을 넣어준다.
          fullData.forEach((el2) => {
            if (el2.treatmentName == el.name) {
        ...
        setObj_avgLeafStage.forEach((el) => {
          //5.이중 forEach써서 name과treatmentName이같으면 avgInternodeLength값을 넣어준다.
          fullData.forEach((el2) => {
            if (el2.treatmentName == el.name) {
        ...
        setObj_avgLeafWidth.forEach((el) => {
          //5.이중 forEach써서 name과treatmentName이같으면 avgInternodeLength값을 넣어준다.
          fullData.forEach((el2) => {
            if (el2.treatmentName == el.name) {
        ...
        setObj_avgStemThickness.forEach((el) => {
          //5.이중 forEach써서 name과treatmentName이같으면 avgInternodeLength값을 넣어준다.
          fullData.forEach((el2) => {
            if (el2.treatmentName == el.name) {
