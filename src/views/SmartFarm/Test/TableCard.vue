...
//import { el: any } from 'vuetify/lib/locale';
...
export default class TableCard extends Vue {
    $refs!: {
        serveyImage: HTMLFormElement;
        myimg: HTMLFormElement;
    }

    tableData: any = [];
    deletePageNum: any = '';
    selectedRow: any = null;
    snackForParents: string = '';
    selectTreatMent: any = [];
    url: any = null;
    delete_item: object = {};
    zoomItem: any = [];
    zoomInDialog: boolean = false;
    imageDatas: any = [];
    fileNames: any = [];
    ...
    @Prop({ required: true }) parentTable: any;
    @Prop({ required: true }) i: any;
    @Prop({ required: true }) ReportId: any;
    ...
    plusAVG() { //this.table에 각 데이터 평균값추가
        ...
        for (let top = 0; top < this.table.carousel_data.length; top++) {
            let reduer = (a: any, b: any) => a + b; //reduce이용해서 평균구하기 
    ...
    openDeleteDialog(table: any, i: any) {
    ...
    openDeleteImageDialog(item: any, child: any) { //사진을 삭제하시겠습니까? 다이아로그
        let fileNameArr = []
    ...
    getSingleImage(item: any, child: any) { //개별이미지 조회 api 
    ...
    zoomInImage(paramImg: any) {
        this.zoomInDialog = true
        this.zoomItem = paramImg
        // 서버에서 받은 이미지 데이터를 인코딩한다
        let images: any = []
        let imagedata: any = paramImg
        console.log('이미지데이타확인', imagedata)
        const contentType: any = "image/png";
        const b64Data: any = imagedata.fileData;
        const image_data: any = atob(b64Data); // base64 인코딩을 풀어준다
        ...
        const blobUrl: any = URL.createObjectURL(blob); // object url 생성
        images.push({
            url: (blobUrl),
    ...
    onInputImage(img: any) {         // 사진 업로드 api가 들어있는 메서드
