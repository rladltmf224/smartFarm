...
export default class OrderManagementModal extends Vue {
    rules: any = {
        quantity: v: any=> !!v || '아이디는 필수 입력사항입니다.',
    }
footer_option: {
...
validCheck_edit() {
    ...
    for (let i = 0; i < this.order.details.length; i++) {
        let temp = this.order.details[i]
        let order = this.order
        if (temp.expectedDeliveryDate == null || temp.quantity == '' || temp.supplyUnitPrice == '' || order.customerId == '' || order.deliveryDate == '' || order.details == null) {
            validYN = true
        }
    }
    if (!validYN) {
