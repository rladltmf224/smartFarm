        ...
        <v-col class="ma-2" cols="12">
          <v-row dense class="mb-2">
            ...
            <v-col class="text-right" cols="3">
              <v-btn color="primary" elevation="0" @click="dialogOpen()"
                >이미지 조회
              </v-btn>
            </v-col>
          </v-row>
...
export default class ShowImage extends Vue {
  ...
  getImageAPI(item: any) {
    //이미지 리스트 api 연결하는 메서드
    this.isLoading = true;
  ...
  makeImageArr() {
    ...
    for (let top = 0; top < data.length; top++) {
      for (let mid = 0; mid < data[top].detailInfo.length; mid++) {
        for (
          let bottom = 0;
          bottom < data[top].detailInfo[mid].fileInfo.length;
          bottom++
        ) {
          let images = [];
