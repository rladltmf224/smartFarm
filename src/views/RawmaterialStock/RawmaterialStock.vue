          ...
          <v-card class="pa-3" height="120">
            <v-row no-gutters>
              <v-col cols="10">
                <v-row>
                  <v-col cols="3">
                    <v-text-field label="품목명 or 품목코드" v-model="search_condition.item" @keydown.enter="getCustomer"
                      dense></v-text-field>
                  </v-col>
                ...
                <v-row no-gutters>
                  <v-col cols="2">
                    <v-select label="창고" v-model="search_condition.storage" :items="storage_list_search" item-text="name"
                      item-value="id" @change="getCustomer" dense></v-select>
                  </v-col>
                  <v-col md="2">
      ...
      <v-row no-gutters>
        <v-col class="ma-2" md="12">
          ...
          <v-card>
            ...
            <v-data-table height="300" :headers="headers" :items="statement_list" item-key="barcode" fixed-header
              multi-sort single-select dense :options.sync="rawmaterialStockOption.options"
              ...
              @page-count="rawmaterialStockOption.pageCount = $event" hide-default-footer>
        ...
        <v-col class="ma-2" md="12">
          ...
          <v-card>
            <v-data-table height="270" :headers="headers_detail" :items="item_list" item-key="barcode" :items-per-page="5"
              hide-default-footer fixed-header multi-sort dense>
